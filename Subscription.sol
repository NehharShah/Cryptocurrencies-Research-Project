...
contract SubscriptionHandler is AccessControl {
    ...
    bytes32 public constant SUBSCRIPTION_PROCESSOR =keccak256("SUBSCRIPTION_PROCESSOR");
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _subscriptionRoles.length; i++) {
            _grantRole(SUBSCRIPTION_PROCESSOR, _subscriptionRoles[uint256(i)]);
        }
    }
    ...
    /// @dev calculate price after deducting hifi fee
    /// @param _price pre tax price

    function calculatehifiPrice(uint256 _price)
        internal
        view
        returns (uint256 hifiPrice)
    {
        return _price - ((_price * hifiFee) / basisPoints);
    }
    ...
    /// @dev Sets fee for hifi tax
    /// @param _hifiFee fee for transaction
    /* fee must be input as out of 1000, ex 10% = 100/1000 */

    function sethifiFee(uint8 _hifiFee) public onlyAdmin {
        hifiFee = _hifiFee;
    }
    ...
    function swapExactOutputMultihop(
        ...
        ISwapRouter.ExactOutputParams memory params = ISwapRouter
            .ExactOutputParams({
                ...
                amountOut: calculatehifiPrice(_amountOut),
                amountInMaximum: calculatehifiPrice(_amountInMaximum)
            });
        ...
        if (amountIn < calculatehifiPrice(_amountInMaximum)) {
            TransferHelper.safeApprove(_tokenIn, address(swapRouter), 0);
            TransferHelper.safeTransfer(
                _tokenIn,
                _payeeAddress,
                calculatehifiPrice(_amountInMaximum) - amountIn
            );
        }
        return amountIn;
    }
        ...
        if (productData.trialPeriod == 0) {
            ...
            if (_tokenAddressIn == productData.tokenOut) {
                ...
                TransferHelper.safeTransfer(
                    ...
                    calculatehifiPrice(productData.amountRecurring)
                );
            } else {
